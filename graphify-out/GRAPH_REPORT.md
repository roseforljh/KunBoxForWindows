...
## Corpus Check
- 54 files · ~62,161 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 561 nodes · 1049 edges · 20 communities detected
...
8. `kernel_download()` - 11 edges
9. `test_latency_via_temp_backend()` - 11 edges
10. `save_profiles_data()` - 11 edges

## Surprising Connections (you probably didn't know these)
- `generate_config()` --calls--> `generate_config_with_settings()`  [EXTRACTED]
  src-tauri\src\commands\singbox.rs → src-tauri\src\commands\singbox.rs  _Bridges community 8 → community 1_
...
Cohesion: 0.04
Nodes (98): active_or_first_node(), allocate_clash_api_port(), append_startup_diagnostic(), apply_route_target(), bounded_selector_probe_helper_caps_concurrency(), build_dns_server(), build_dns_server_with_resolver(), build_foreign_wintun_warning() (+90 more)
...
Cohesion: 0.06
Nodes (20): custom_rules_get(), custom_rules_save(), domain_rules_get(), domain_rules_save(), load_custom_rules(), save_custom_rules(), decode_windows_output(), ensure_u16_in_range() (+12 more)
...
Cohesion: 0.42
Nodes (13): generate_config(), generate_config_bridges_xhttp_nodes_through_local_xray_plugin(), generate_config_keeps_proxy_dns_for_non_ech_active_node(), generate_config_preserves_profile_scoped_node_identity_for_domain_rules(), generate_config_routes_xray_plugin_remote_direct_in_tun_mode(), generate_config_supports_profile_scoped_and_bare_ruleset_node_routes(), generate_config_uses_direct_dns_bootstrap_for_ech_nodes(), generate_config_uses_profile_ids_for_selector_tags() (+5 more)
...
- **Thin community `Community 13`** (2 nodes): `manual_test.rs`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 14`** (1 nodes): `ast_extract.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 17`** (1 nodes): `vite.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 18`** (1 nodes): `global.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
